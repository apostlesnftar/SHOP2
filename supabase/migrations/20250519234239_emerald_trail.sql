/*
  ...
  1. Changes
    - Update process_friend_payment function to properly handle custom payment methods
    - Fix validation logic to accept all payment methods from payment_gateways table
    - Improve error handling and logging
*/

-- Update process_friend_payment function to handle custom payment methods
...
BEGIN
  ...
  -- This query checks if the payment method exists in payment_gateways table
  -- It handles custom providers and built-in methods like acacia_pay
  SELECT EXISTS (
    SELECT 1 FROM (
