/*
  ...
  1. New Functions
    - `create_team_member`: Creates a new agent as a team member under a parent agent
      - Handles user creation in auth.users
      - Creates user_profile with agent role
      - Sets up agent record with proper parent-child relationship
      - Validates inputs and handles errors properly
  
  2. Security
    - Function is security definer to run with elevated privileges
    - Proper input validation to prevent abuse
