/*
  ...
  1. Changes
    ...
    - Add wallet transaction records for commission payments
    - Handle both direct referrals and agent-customer relationships
    - Fix trigger already exists error by checking before creating
*/
...
-- Check if the trigger exists before trying to create it
DO $$
BEGIN
  -- Drop existing trigger if it exists
  IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'on_order_processing') THEN
    DROP TRIGGER on_order_processing ON orders;
  END IF;
  
  -- Drop the old trigger if it exists
  IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'on_order_delivered') THEN
