...
-- Use DO block to check if triggers exist before trying to create them
DO $$
DECLARE
  trigger_exists BOOLEAN;
BEGIN
  -- Check if on_order_processing trigger exists
  SELECT EXISTS (
    SELECT 1 
    FROM pg_trigger 
    WHERE tgname = 'on_order_processing' 
    AND tgrelid = 'orders'::regclass
  ) INTO trigger_exists;
  
  -- Only drop if it exists
  IF trigger_exists THEN
    EXECUTE 'DROP TRIGGER on_order_processing ON orders';
  END IF;
  
  -- Check if on_order_delivered trigger exists
  SELECT EXISTS (
    SELECT 1 
    FROM pg_trigger 
    WHERE tgname = 'on_order_delivered' 
    AND tgrelid = 'orders'::regclass
  ) INTO trigger_exists;
  
  -- Only drop if it exists
  IF trigger_exists THEN
    EXECUTE 'DROP TRIGGER on_order_delivered ON orders';
  END IF;
END $$;
