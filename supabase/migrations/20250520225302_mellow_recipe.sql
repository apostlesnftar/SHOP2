/*
  ...
  1. Changes
    - Fix the "column reference 'referrer_id' is ambiguous" error in the process_friend_payment function
...
BEGIN
  ...
  -- Store user_id and order_total for later use
  v_user_id := v_shared_order.user_id;
  v_order_total := v_shared_order.order_total;
  v_order_id := v_shared_order.order_id;
  v_order_number := v_shared_order.order_number;
  ...
  -- Get the referrer_id with explicit table alias to avoid ambiguity
