/*
  # Add referrer_id to shared_orders table
  
  1. Changes
    - Add referrer_id column to shared_orders table
    - Update share_order function to store referrer_id when creating shared orders
    - Fix process_friend_payment function to properly handle referrer_id
...
-- Add referrer_id column to shared_orders table if it doesn't exist
DO $$ 
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE shared_orders ADD COLUMN referrer_id UUID REFERENCES user_profiles(id);
  END IF;
END $$;

-- Create index for better query performance
CREATE INDEX IF NOT EXISTS idx_shared_orders_referrer_id ON shared_orders(referrer_id);

-- Update share_order function to store referrer_id
...
DECLARE
  ...
  v_referrer_id uuid; -- Variable for referrer_id
BEGIN
  -- Get order details
  SELECT 
    o.user_id,
  ...
  INSERT INTO shared_orders (
    ...
    order_id,
    expires_at,
    referrer_id -- Include referrer_id
  ) VALUES (
    v_share_id,
    p_order_id,
    v_expires_at,
    v_referrer_id -- Store referrer_id
  )
  ...
  DO UPDATE SET
    ...
    status = 'pending',
    referrer_id = v_referrer_id; -- Update referrer_id
...
-- Update process_friend_payment function to use referrer_id from shared_orders
