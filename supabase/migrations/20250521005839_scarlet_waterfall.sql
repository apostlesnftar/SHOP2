...
DECLARE
  ...
  v_shared_order_referrer_id uuid; -- Even more explicit naming to avoid any ambiguity
  v_order_number text;
...
BEGIN
  ...
  SELECT 
    so.order_id,
    so.referrer_id AS shared_order_referrer_id, -- Very explicit alias to avoid any ambiguity
    COALESCE(so.order_number, o.order_number) AS order_number, -- Use COALESCE to handle NULL
    o.user_id,
    o.total
  INTO 
    v_order_id,
    v_shared_order_referrer_id, -- Store in very explicitly named variable
    v_order_number,
    v_user_id,
    v_order_total
