-- Create helper function to check if an order is a valid shared order
CREATE OR REPLACE FUNCTION is_valid_shared_order(order_id uuid)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
-- Add policies for shared order payment processing only if they don't exist
DO $$
BEGIN
  ...
  IF NOT EXISTS (
    ...
    EXECUTE 'CREATE POLICY "Allow anon select on orders" ON orders FOR SELECT TO anon USING (true)';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    EXECUTE 'CREATE POLICY "Allow anon update on orders" ON orders FOR UPDATE TO anon USING (true)';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    EXECUTE 'CREATE POLICY "Allow anon select on shared_orders" ON shared_orders FOR SELECT TO anon USING (true)';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    EXECUTE 'CREATE POLICY "Allow anon update on shared_orders" ON shared_orders FOR UPDATE TO anon USING (true)';
  END IF;
END
$$;

-- Update the existing RPC function to handle shared orders
