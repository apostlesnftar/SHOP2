/*
  # Fix ambiguous referrer_id column reference in process_friend_payment function
  
  1. Changes
    - Drop existing process_friend_payment function
    - Recreate with explicit table aliases for all column references
    - Use variable names that avoid ambiguity with column names
...
BEGIN
  ...
  -- Use provided referrer_id if available, otherwise get from user_profiles
  IF p_referrer_id IS NOT NULL THEN
    v_user_profile_referrer_id := p_referrer_id;
  ELSE
...
GRANT EXECUTE ON FUNCTION process_friend_payment(text, text, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION process_friend_payment(text, text, uuid) TO anon;